package org.cmucreatelab.android.flutterprek.activities.teacher_section.highlights_design;

import android.graphics.Color;

public class ColorConstants {

    public static String HEX_HAPPY = "#FEDA5A";
    public static String HEX_MAD = "#F1726E";
    public static String HEX_SAD = "#75ADAE";
    public static String HEX_SCARED = "#136782";
    public static String HEX_EXCITED = "#FFC60F";

    public static int HAPPY_COLOR = Color.parseColor(HEX_HAPPY);
    public static int MAD_COLOR = Color.parseColor(HEX_MAD);
    public static int SAD_COLOR = Color.parseColor(HEX_SAD);
    public static int SCARED_COLOR = Color.parseColor(HEX_SCARED);
    public static int EXCITED_COLOR = Color.parseColor(HEX_EXCITED);
}
