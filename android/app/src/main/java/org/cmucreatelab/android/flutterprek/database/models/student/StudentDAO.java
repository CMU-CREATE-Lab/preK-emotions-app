...
public interface StudentDAO {
    ...
    @Query("SELECT * FROM students ORDER BY sequence_id ASC, lower(name) ASC")
    LiveData<List<Student>> getAllStudents();

    @Query("SELECT * FROM students WHERE classroom_uuid = :classroomUuid ORDER BY sequence_id ASC, lower(name) ASC")
    ...
    @Query("SELECT * FROM students WHERE uuid IN (:studentUuids) ORDER BY sequence_id ASC, lower(name) ASC")
    LiveData<List<StudentWithCustomizations>> getAllStudentsWithCustomizationsFromList(List<String> studentUuids);
